...
export default function TaskForm({ onSave, onClose, allTasks, taskToEdit, parentId = null }) {
    ...
    // --- Lógica para prevenir dependencias circulares ---
    ...
    // ¡NUEVO! Función para encontrar todas las tareas que dependen de esta (sucesores)
    ...
    const availableDependencies = useMemo(() => {
        ...
        const successors = getSuccessors(taskToEdit.id, allTasks); // Obtenemos los sucesores
        
        // Añadimos los sucesores a la lista de IDs inválidos
        const invalidIds = new Set([taskToEdit.id, ...ancestors, ...descendants, ...successors]);
                            ...
                            {dependencies.map(depId => {
                                ...
                                return (
                                    <span key={depId} className={`flex items-center gap-2 px-2 py-1 text-xs rounded-full ${isCritical ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' : 'bg-sky-100 text-sky-800 dark:bg-sky-900 dark:text-sky-200'}`}>
                                        ...
                                        {depTask.title}
                                        <button type="button" onClick={() => handleRemoveDependency(depId)} className="hover:text-red-500">
